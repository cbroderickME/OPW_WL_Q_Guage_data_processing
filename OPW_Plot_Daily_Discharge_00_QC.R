...
base_dir <- "C:/Users/CBroderick/Downloads/OPW/Data/Q_Output_2026-04-05/Gauges"
...
for (gauge_dir in gauge_dirs) {
  ...
  tryCatch({
    ...
    file_path <- file.path(gauge_dir, "Raw_Discharge.rds")
    ...
    dt[, Use_Flag := fifelse(
      !is.na(Discharge) & !Quality %in% exclude_qc,
      1L, 0L
    )]
    ...
    p1 <- plot_ly(daily_all, x = ~Date) %>%
      add_lines(y = ~Raw_Discharge, name = "Raw Daily Mean", line = list(color = "black"),
      ...
      add_lines(y = ~Filtered_Discharge, name = "Filtered Daily Mean", line = list(color = "blue"),
                customdata = ~Dominant_QC,
                hovertemplate = paste(
                  "Filtered<br>%{x}<br>",
                  "Discharge: %{y:.2f} m³/s<br>",
