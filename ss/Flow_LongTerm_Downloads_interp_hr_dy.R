...
for (val in seq(from=1, to=length(gauge_lst))){
  ...
  tryCatch({
    ...
    gen_ts = rev(seq(ymd_hm('2000-01-01 00:00', tz = "GMT"), Sys.time()+days(15), by = '15 mins'))
    ...
    # ---- Daily average from 9AM to next day 9AM with minimum 20 valid hours ----
    
    # Create output directory for 9AM daily averages
    daily_avg_dir_9am <- paste0(dest_dir, "/Daily_9am_Avg")
    if (!dir.exists(daily_avg_dir_9am)) dir.create(daily_avg_dir_9am)
    
    # Shift timestamp by -9 hours so that flooring groups 9AM to 9AM period
    comp_series_hourly <- comp_series_hourly %>%
      mutate(
        ts_shifted = Timestamp_interp - hours(9),
        day_9am = as.Date(ts_shifted)
      )
    
    # Calculate daily mean with min 20 valid hours condition for 9AM day
    daily_9am_avg <- comp_series_hourly %>%
      group_by(day_9am) %>%
      summarise(
        n_obs = sum(!is.na(Value)),
        Mean_Discharge = if_else(n_obs >= 20,
                                 mean(Value, na.rm = TRUE),
                                 NA_real_)
      ) %>%
      ungroup() %>%
      mutate(
        Timestamp_9am = as.POSIXct(paste0(day_9am, " 09:00:00"), tz = "GMT")
      ) %>%
      select(Timestamp_9am, Mean_Discharge, n_obs)
    
    # Save 9AM daily average data
    saveRDS(daily_9am_avg, file = paste0(daily_avg_dir_9am, "/Daily_9am_Avg_Discharge.rds"))
    write.csv(daily_9am_avg, file = paste0(daily_avg_dir_9am, "/Daily_9am_Avg_Discharge.csv"), row.names = FALSE)
    
    
    # ---- Daily average from midnight (00:00) to midnight (00:00) with minimum 20 valid hours ----
    
    # Create output directory for midnight daily averages
    daily_avg_dir_00 <- paste0(dest_dir, "/Daily_00_Avg")
    if (!dir.exists(daily_avg_dir_00)) dir.create(daily_avg_dir_00)
    
    # Extract date based on Timestamp_interp without any shift (midnight to midnight)
    comp_series_hourly <- comp_series_hourly %>%
      mutate(day_00 = as.Date(Timestamp_interp))
    
    # Calculate daily mean with min 20 valid hours condition for midnight day
    daily_00_avg <- comp_series_hourly %>%
      group_by(day_00) %>%
      summarise(
        n_obs = sum(!is.na(Value)),
        Mean_Discharge = if_else(n_obs >= 20,
      ...
      ungroup() %>%
      mutate(
        Timestamp_00 = as.POSIXct(paste0(day_00, " 00:00:00"), tz = "GMT")
      ) %>%
      select(Timestamp_00, Mean_Discharge, n_obs)
    
    # Save midnight daily average data
    saveRDS(daily_00_avg, file = paste0(daily_avg_dir_00, "/Daily_00_Avg_Discharge.rds"))
    write.csv(daily_00_avg, file = paste0(daily_avg_dir_00, "/Daily_00_Avg_Discharge.csv"), row.names = FALSE)
