################################################################################
## OPW Long-Term Discharge Downloader – EPA-Style Streamlined (Optimized)
################################################################################
...
wkdir <- "C:/Users/CBroderick/Downloads/OPW"
...
for(val in seq_along(gauge_lst)) {
  ...
  tryCatch({
    ...
    comp_raw[, Use_Flag := fifelse(
      !is.na(Discharge) &
        Quality %in% c(31,32,36,56,96,254) &
        year(Timestamp) >= start_year,
      1L, 0L
    )]
    
    dt <- comp_raw[Use_Flag==1, .(Timestamp, Discharge, Quality)]
    ...
    dt_full[, Discharge := zoo::na.approx(Discharge, x=Timestamp, maxgap=1, na.rm=FALSE)]
    log_tbl$Prop_Missing_15min[val] <- mean(is.na(dt_full$Discharge))
    ...
    hourly_dt <- dt_full[, .(
      ...
      Discharge = if(.N - sum(!is.na(Discharge)) <= max_missing_hourly) mean(Discharge, na.rm=TRUE) else NA_real_
    ), by=Hour]
    ...
    log_tbl$Prop_Missing_Hourly[val] <- mean(is.na(hourly_dt$Discharge))
    ...
    daily_dt <- dt_full[, .(
      n_total = .N,
      n_valid = sum(!is.na(Discharge)),
      Discharge = if(.N - sum(!is.na(Discharge)) <= max_missing_daily) mean(Discharge, na.rm=TRUE) else NA_real_
    ), by=Date_09]
    ...
    log_tbl$Prop_Missing_Daily_09_09[val] <- mean(is.na(daily_dt$Discharge))
    
    ################################################################################
    # SAVE RDS
    ################################################################################
    saveRDS(comp_raw, file.path(dest_dir,"Raw_Discharge.rds"))
    saveRDS(dt_full, file.path(dest_dir,"Regular_15min_Discharge.rds"))
    saveRDS(hourly_dt, file.path(dest_dir,"Hourly_Discharge.rds"))
    saveRDS(daily_dt, file.path(dest_dir,"Daily_Discharge_09_09.rds"))
    ...
    p <- add_trace(p,
                   ...
                   name = "Raw (Unfiltered)",
                   hovertemplate="Date: %{x}<br>Discharge: %{y:.2f} m³/s<br>QC Code: %{customdata}<extra></extra>",
                   customdata = ~Quality)
