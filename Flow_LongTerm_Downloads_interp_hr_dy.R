...
#wkdir <- "K:/Fluvial/Hydrometric_Network/OPW_Gauges/Dwnld_LongTerm_Hydo_OPW"
...
for (val in seq(from=1, to=length(gauge_lst))){
  ...
  tryCatch({
    ...
    gen_ts = rev(seq(ymd_hm('1990-01-01 00:00', tz = "GMT"), Sys.time()+days(15), by = '15 mins'))
    ...
    ### DAILY MEAN DISCHARGE BETWEEN 09:00 DAY 1 AND 09:00 DAY 2     ### Timestamped as DAY 1     ### No interpolation, minimum 18 hours of valid data
    ...
    saveRDS(comp_series_daily, "Daily_Discharge_09_09.rds")
    write.csv(comp_series_daily, "Daily_Discharge_09_09.csv", row.names = FALSE)
